...
_mnemo() {
  ...
  case "\${cmd}" in
    ...
    mcp)
      ...
      ;;
    workspace)
      COMPREPLY=( $(compgen -W "list new create switch rename set-storage archive restore delete --json --no-json" -- "\${cur}") )
      ;;
    sync)
      if [[ \${COMP_CWORD} -eq 2 ]]; then
...
_mnemo() {
  ...
  case "\${words[2]}" in
    ...
    workspace)
      _arguments '--json' '--no-json' \
        '1:subcommand:(list new create switch rename set-storage archive restore delete)'
      ;;
    sync)
      if (( CURRENT == 3 )); then
