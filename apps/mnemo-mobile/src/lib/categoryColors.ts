import { GENERAL_PATH, UNASSIGNED_PATH, categoryColorStorageKey, normalizePath, parentPath } from './categoryPath';
import type { Theme } from '../theme/theme';

/** Resolve display color: exact match, then walk up to parent folders (desktop parity). */
export function colorForCategoryPath(path: string, colors: Record<string, string>): string | undefined {
...
function isReadableOnPanel(hex: string, panelBg: string, minRatio = 3): boolean {
...
function hslToHex(h: number, s: number, l: number): string {
  ...
  const c255 = (n: number) =>
    Math.max(0, Math.min(255, Math.round(n)))
      .toString(16)
      .padStart(2, '0');
  return `#${c255((rp + m) * 255)}${c255((gp + m) * 255)}${c255((bp + m) * 255)}`;
}

function tweakForContrast(hex: string, panelBg: string, isDark: boolean): string {
  let { h, s, l } = hexToHsl(hex);
  for (let i = 0; i < 8; i++) {
    if (isReadableOnPanel(hslToHex(h, s, l), panelBg)) return hslToHex(h, s, l);
...
/** Theme-aware swatches for pickers (desktop getSuggestedCategorySwatches analogue). */
export function getSuggestedCategorySwatches(theme: Theme): string[] {
  const panelBg = theme.card;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!isReadableOnPanel(hex, panelBg)) {
...
function pickAutoColorForKey(key: string, swatches: string[], accentFallback: string, panelBg: string): string {
  const pBg = parseHexChannels(panelBg);
  const isDark = pBg ? relativeLuminance(pBg) < 0.45 : true;
  if (swatches.length === 0) {
    return isReadableOnPanel(accentFallback, panelBg) ? accentFallback : tweakForContrast(accentFallback, panelBg, isDark);
  }
  const i = hashPathKey(key) % swatches.length;
  return swatches[i]!;
}

/** Merge explicit picks with auto colors for paths without a stored hex (sidebar parity). */
export function buildEffectiveCategoryColors(
  explicit: Record<string, string>,
  theme: Theme,
): Record<string, string> {
  const panelBg = theme.card;
  const accentFallback = theme.primary;
...
/** Stripe color when no explicit/inherited hex: deterministic hue from path. */
export function colorForCategoryPathOrAuto(
  path: string,
  effective: Record<string, string>,
  theme: Theme,
): string {
  const c = colorForCategoryPath(path, effective);
  if (c) return c;
  const swatches = getSuggestedCategorySwatches(theme);
  const key = categoryColorStorageKey(path);
  return pickAutoColorForKey(key, swatches, theme.primary, theme.card);
}
