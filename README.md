...
- **Desktop app (Electron)** — Markdown editor, graph, IDE/sidebar layouts, Settings for remote DB
- **CLI** — `mnemo note …` for list/search/show/new/import, categories, link graph, autolink
- **MCP** — stdio server (`mnemo mcp`) for Cursor / Claude Desktop; HTTP/SSE (`mnemo mcp-http`) for remote libSQL + bearer auth
