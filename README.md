...
3. **Build locally (Windows):** From the repo root:
   ...
   npm run make
   ```
4. **Build in CI (no Windows machine):** The workflow **Build Windows installer** (`.github/workflows/windows-build.yml`) runs on **`windows-latest`**, executes **`npm ci`** and **`npm run make`**, and uploads everything under **`out/make/`** as a workflow artifact (retained 90 days). In GitHub: **Actions** → select a successful run → scroll to **Artifacts** → download **`mnemo-windows-…`**. For **official releases**, push a **`v*`** tag and use the **Release** workflow instead — it attaches **`mnemo-<tag>-windows-x64.zip`** (and the Linux zip) directly to the [GitHub Releases](https://github.com/fwgadmin/mnemo/releases) page (see **GitHub releases (automated)** above). Triggers for the Windows-only workflow: **workflow_dispatch**, pushes to **`main`** / **`master`** that change listed paths, and pull requests targeting those branches.
